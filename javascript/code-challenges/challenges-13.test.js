...
// Solution code here...
  let str = arr.reduce((a,b) => a.length > b.length ? a : b ,arr[0]);
...
const findHappiness = (arr) => {
  // Solution code here...
  let smile = ':)';
  let newArr = arr.filter(str => str.includes(smile));
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let regex = /\d/g;
  return arr.map(str => str.match(regex).join(''));
  // console.log(newArr);
};
...
const allHappy = (arr) => {
  // Solution code here...
  let smile = ':)';
  return arr.every(str => str.includes(smile));
};
