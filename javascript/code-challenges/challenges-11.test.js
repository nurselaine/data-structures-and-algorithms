...
function transformToLis(obj){
  // Solution code here...
  return Object.keys(obj).map(prop =>`<li>${prop}: ` + obj[prop] + `</li>`);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((total, value) => {
    return total + value;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  // let newarr = input.map(arr => arr.filter(num => typeof(num) !== 'number' || num % 5 !== 0))
  // .map(num => Math.pow(num, 2));
  let newarr = input.map(arr => arr.filter(arr => typeof(arr) === 'number'));
  newarr = newarr.map(arr => arr.filter(num => num % 5 === 0)).map(arr => arr.map(elem => Math.pow(2, elem)));
  return newarr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newData = data.filter(obj => obj.gender === 'male' || obj.gender === 'female').map(obj => obj.name).join(' and ');
  return newData;
};
